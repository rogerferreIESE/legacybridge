...
const ValuationCalculator: React.FC = () => {
    ...
    const [marketComp, setMarketComp] = useState(0); // What a replacement CEO would cost
    const [discretionaryAddBacks, setDiscretionaryAddBacks] = useState(0);
    ...
    const ownerCompAdjustment = ownerComp > marketComp ? ownerComp - marketComp : 0; // Add back excess compensation conditionally
    const totalAddBacks = oneTimeExpenses + ownerCompAdjustment + discretionaryAddBacks;
            ...
            <div className="valuation-grid">
                ...
                <div className="glass-panel input-panel">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                        <h3 style={{ margin: 0 }}>Financial Data</h3>
                        <div className="currency-toggle" style={{ display: 'flex', background: 'rgba(255,255,255,0.05)', borderRadius: '6px', padding: '2px' }}>
                            {currencyOptions.map(currency => (
                                <button
                                    key={currency.code}
                                    onClick={() => setSelectedCurrency(currency)}
                                    style={{
                                        background: selectedCurrency.code === currency.code ? 'var(--primary)' : 'transparent',
                                        color: selectedCurrency.code === currency.code ? 'white' : 'var(--text-muted)',
                                        border: 'none',
                                        padding: '4px 12px',
                                        borderRadius: '4px',
                                        fontSize: '0.8rem',
                    ...
                    <div className="input-group tooltip-container">
                        <label>
                            Annual Revenue
                            <span className="info-icon" title="Trailing Twelve Months (TTM) or Last Full Year Revenue">ⓘ</span>
                        </label>
                        <div className="range-slider-wrapper">
                            <input
                                type="range"
                                min="0" max="15000000" step="100000"
                                value={revenue}
                    ...
                    <div className="input-group">
                        <label>Discretionary Add-backs</label>
                        <input
                            type="number"
                            className="number-input"
                            value={discretionaryAddBacks}
                            onChange={(e) => setDiscretionaryAddBacks(Number(e.target.value))}
                        />
                        <small>e.g. Non-essential operational expenses</small>
                    </div>
                    ...
                    <div className="input-group owner-comp-group">
                        ...
                        <div className="tooltip-container">
                            <label>
                                ...
                                <span className="info-icon" title="The estimated salary required to hire a CEO to replace you">ⓘ</span>
                            </label>
                            <input
                                type="number"
                                className="number-input"
                ...
                <div className="glass-panel results-panel">
                    ...
                    <div className="valuation-final">
                        ...
                        <p className="disclaimer" style={{ color: 'var(--text)', marginBottom: '0.5rem' }}>Enterprise Value is calculated by applying an adjusted market multiple to your true normalized EBITDA.</p>
